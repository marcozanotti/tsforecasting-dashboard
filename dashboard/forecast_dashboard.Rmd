...
output: 
  flexdashboard::flex_dashboard:
    ...
    navbar:
      ...
      - { title: "Manual", href: "https://marcozanotti.github.io/tsforecasting-dashboard/manual/forecastdash-manual.html", align: right }
runtime: shiny
---
...
logging::addHandler(logging::writeToFile, file = "forecast_dashboard.log")
...
sliderInput(inputId = "alpha", label = "Confidence Level", min = 0.01, max = 1, value = 0.02, step = 0.01)
sliderInput(inputId = "max_anomalies", label = "Max Anomalies (%)", min = 0.01, max = 1, value = 0.1, step = 0.01)

hr()
h4("Anomaly Cleaning")
shinyWidgets::switchInput(inputId = "clean", value = FALSE, size = "mini")
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updateSwitchInput(session = session, inputId = "clean", value = FALSE)
  }
)
...
h6("The dataset must be a .csv file containing columns 'date', 'id', 'frequency' and the desired external features.")
fileInput(
...
br()
sliderInput(
	inputId = "featsel_cor_thresh", label = "Absolute Correlation Threshold",
...
data_features_full <- reactive({
	...
	if (!is.null(react_val$uploaded_xfeatures)) {
		req(data_xfeatures())
		data_features() |> dplyr::left_join(data_xfeatures(), by = "date") # c("date", "id", "frequency")
	} else {
		data_features()
	}
})
...
output$featsel_corr_plot <- plotly::renderPlotly({
	...
	g <- data_importance() |> 
		...
		plot_feature_importance() + ggplot2::expand_limits(x = c(-1, 1))
	plotly::ggplotly(g)
})
...
output$featsel_ppsr_plot <- plotly::renderPlotly({
	...
	g <- data_importance() |> 
		...
		plot_feature_importance() + ggplot2::expand_limits(x = c(0, 1))
	plotly::ggplotly(g)
})
...
output$featsel_table <- DT::renderDT({
...
```{r}
DT::DTOutput(outputId = "featsel_table")
```
...
output$scn_wb_table <- DT::renderDT({
  back_scenario_results()$oos_forecast |>
    ...
    dplyr::mutate(across(.cols = where(is.numeric), .fns = ~ round(., 2))) |> 
