...
set_options()
# Sys.setenv(JAVA_HOME = "/usr/lib/jvm/jdk-17/") # /usr/lib/jvm/java-17-openjdk-amd64/
...
output$data_str <- renderPrint({ glimpse(data_selected()) })
...
awesomeCheckbox(inputId = "feat_calendar", label = "Add calendar features?", value = FALSE)
awesomeCheckbox(inputId = "feat_holiday", label = "Add holiday features?", value = FALSE)
hr()
textInput(
...
output$feat_ts_plot <- renderPlotly({
	if (get_features(data_features(), number_only = TRUE) > 1) {
...
```{r}
plotlyOutput(outputId = "feat_ts_plot")
```
...
output$xfeat_str <- renderPrint({ glimpse(data_xfeatures()) })
output$xfeat_summ <- renderPrint({ skim(data_xfeatures()) })

output$xfeat_ts_plot <- renderPlotly({
	req(data_xfeatures())
  data_transformed() |>
...
output$xfeat_crosscorr_plot <- renderPlotly({
	req(data_xfeatures())
  data_transformed() |>
    add_future_frame(input$xfeat_n_future) |> 
  	dplyr::left_join(data_xfeatures(), by = c("date", "id", "frequency")) |> 
...
```{r}
plotlyOutput(outputId = "xfeat_ts_plot")
```

### Cross Correlation {.no-padding}

```{r}
plotlyOutput(outputId = "xfeat_crosscorr_plot")
```
...
br()
awesomeCheckbox(inputId = "featsel_save", label = "Save feature set?", value = FALSE)
```
...
output$featsel_corr_plot <- renderPlotly({
	req(data_importance())
	g <- data_importance() |> 
...
output$featsel_ppsr_plot <- renderPlotly({
	req(data_importance())
	g <- data_importance() |> 
...
output$featsel_lasso_plot <- renderPlotly({
	req(data_importance())
	g <- data_importance() |> 
...
output$featsel_rf_plot <- renderPlotly({
	req(data_importance())
	g <- data_importance() |> 
...
output$featsel_reg_plot <- renderPlotly({
...
```{r}
plotlyOutput(outputId = "featsel_corr_plot")
```

### Predictive Power Score {.no-padding}

```{r}
plotlyOutput(outputId = "featsel_ppsr_plot")
```

### LASSO {.no-padding}

```{r}
plotlyOutput(outputId = "featsel_lasso_plot")
```

### Random Forest {.no-padding}

```{r}
plotlyOutput(outputId = "featsel_rf_plot")
```

### Selected Feature Set

```{r}
DTOutput(outputId = "featsel_table")
```

### Regression Analysis {.no-padding}

```{r}
plotlyOutput(outputId = "featsel_reg_plot")
```
...
dropdownButton(
  ...
  awesomeCheckbox(inputId = "use_feat_set", label = "Use selected fetures?", value = FALSE),
  icon = icon("gear"), width = "225px", size = "sm", circle = TRUE,
  tooltip = tooltipOptions(title = "Click to see inputs!")
)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateAwesomeCheckbox(session = session, inputId = "use_feat_set", value = FALSE)
...
react_val <- reactiveValues(
	tune_fit = purrr::set_names(
		vector("list", length(tune_methods)), 
		tune_methods
	)
)
  ...
  handlerExpr = {
    req(data_feat_selected())
  	if (get_features(data_feat_selected()$data, remove_date = TRUE, number_only = TRUE) > 0) {
  		updateAwesomeCheckbox(inputId = "use_feat_set", session = session, value = TRUE)
