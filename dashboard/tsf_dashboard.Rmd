...
materialSwitch(inputId = "test_boxcox", label = "Test on Box-Cox?", value = FALSE, status = "primary")
materialSwitch(inputId = "test_norm", label = "Test on normalized?", value = FALSE, status = "primary")
materialSwitch(inputId = "test_stand", label = "Test on standardized?", value = FALSE, status = "primary")
  ...
  data_imputed() |> 
    transform_data(
    	transformations = list(
    		"log" = input$test_log, "boxcox" = input$test_boxcox, 
    		"norm" = input$test_norm, "stand" = input$test_stand, 
    		"diff" = input$test_diff, "sdiff" = input$test_sdiff
    	), 
    	freq = ts_freq()
    )
})
...
materialSwitch(inputId = "transf_log", label = "Log", value = FALSE, status = "primary")
materialSwitch(inputId = "transf_boxcox", label = "Box-Cox", value = FALSE, status = "primary")
materialSwitch(inputId = "transf_norm", label = "Normalize", value = FALSE, status = "primary")
materialSwitch(inputId = "transf_stand", label = "Standardize", value = FALSE, status = "primary")
materialSwitch(inputId = "transf_diff", label = "Diff", value = FALSE, status = "primary")
materialSwitch(inputId = "transf_sdiff", label = "Seasonal Diff", value = FALSE, status = "primary")
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateNumericInput(session = session, inputId = "transf_acf_lag", value = ifelse(ts_freq() < 60, ts_freq() * 3, ts_freq()))
    updateMaterialSwitch(session = session, inputId = "transf_log", value = FALSE)
    updateMaterialSwitch(session = session, inputId = "transf_boxcox", value = FALSE)
    updateMaterialSwitch(session = session, inputId = "transf_norm", value = FALSE)
    updateMaterialSwitch(session = session, inputId = "transf_stand", value = FALSE)
    updateMaterialSwitch(session = session, inputId = "transf_diff", value = FALSE)
    updateMaterialSwitch(session = session, inputId = "transf_sdiff", value = FALSE)
  }
)
```
...
data_transformed <- reactive({
  data_cleaned() |> 
    transform_data(
    	transformations = list(
    		"log" = input$transf_log, "boxcox" = input$transf_boxcox, 
    		"norm" = input$transf_norm, "stand" = input$transf_stand, 
    		"diff" = input$transf_diff, "sdiff" = input$transf_sdiff
    	), 
    	freq = ts_freq()
    ) |> 
...
```{r}
exp_forecast_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      ~ fit_model(
        data = data_transformed(), method = ., params = input, 
        n_assess = input$exp_n_assess, assess_type = input$exp_assess_type, seed = 1992
      )
    ) |> 
      generate_forecast(
        data = data_transformed(), method = input$exp_method, n_future = input$exp_n_future,
...
observeEvent(
  eventExpr = input$exp_apply_forecast,
  handlerExpr = {
  	req(exp_forecast_results())
    updatePickerInput(
    	session = session, inputId = "exp_features",
    	choices = get_features(exp_forecast_results()$fit[[1]], names_only = TRUE),
    	selected = get_features(exp_forecast_results()$fit[[1]], names_only = TRUE)[2]
    )
  }
)
...
output$exp_features_table <- renderDT({
  req(exp_forecast_results()) 
	tibble::tibble(
   	get_features(exp_forecast_results()$fit[[1]], names_only = TRUE)
   ) |>
		datatable(
      options = list(
...
output$plot_exp_test_forecast <- renderPlotly({
	req(exp_forecast_results())
  exp_forecast_results()$test_forecast |>
    plot_modeltime_forecast(.legend_show = FALSE, .title = FALSE, .conf_interval_fill = "lightblue")
})
...
output$plot_exp_bd <- renderPlot({
	...
	req(explainer_results())
	req(input$exp_date)
	exp_obs <- get_observation(
		data = data_transformed(), date = input$exp_date, method = input$exp_method,
		n_assess = input$exp_n_assess, assess_type = input$exp_assess_type
	)
	explainer_results() |> 
		explain_model(type = "break_down", observation = exp_obs) |>
		plot() + labs(title = NULL, subtitle = NULL, caption = NULL)
})
...
output$plot_exp_locstab <- renderPlot({
	...
	explainer_results() |> 
		explain_model(type = "local_stability",	observation = exp_obs, features = input$exp_features) |>
		plot() + labs(title = NULL, subtitle = NULL, caption = NULL)
})
```
...
### Test Forecasts {data-height=320 .no-padding}

```{r}
plotlyOutput(outputId = "plot_exp_test_forecast")
```

### Feature Importance {data-height=680 .no-padding}
...
### Variable Response Analysis {.no-padding}
...
output$plot_scn_oos_forecast_wb <- renderPlotly({
  g <- scenario_results()$oos_forecast |> 
    adjust_forecast(input$scn_adjust) |>
    filter(.key == "actual" | .conf_lvl == as.character(input$scn_conf_lvl)) |> 
