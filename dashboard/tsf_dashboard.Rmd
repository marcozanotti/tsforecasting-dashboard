...
pickerInput(
  ...
  choices = transf[1:4], selected = NULL
)
...
output$tune_accuracy_table <- renderDT({
		back_transform_accuracy(
			data = data_transformed(), 
			calibration_table = tune_results()$calibration, 
			n_assess = input$tune_n_assess, assess_type = input$tune_assess_type,
			transform = input$tune_back_transf,
			transformations = input$transf, 
			transform_params = transform_params(), 
...
output$comp_accuracy_table <- renderDT({
	back_transform_accuracy(
			data = data_transformed(), 
			calibration_table = compare_results()$calibration, 
			n_assess = input$comp_n_assess, assess_type = input$comp_assess_type,
			transform = input$comp_back_transf,
			transformations = input$transf, 
			transform_params = transform_params(), 
...
output$plot_exp_test_forecast <- renderPlotly({
  ...
  exp_forecast_results()$test_forecast |>
  	back_transform_data(
			transform = input$exp_back_transf, 
			cols_to_transform = c(".value", ".conf_lo", ".conf_hi"), 
			transformations = input$transf, 
			transform_params = transform_params(), 
			frequency = ts_freq()
...
output$ens_accuracy_table <- renderDT({
  req(react_val$combine_results$calibration)
  back_transform_accuracy(
  	data = data_transformed(),
  	calibration_table = react_val$combine_results$calibration,
...
output$scn_accuracy_table <- renderDT({
	back_transform_accuracy(
		data = data_transformed(),
		calibration_table = scenario_results()$calibration,
		n_assess = input$scn_n_assess, assess_type = input$scn_assess_type,
		transform = input$scn_back_transf,
		transformations = input$transf,
		transform_params = transform_params(),
