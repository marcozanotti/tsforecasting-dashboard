...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateSelectInput(session = session, inputId = "exp_method", selected = "ETS")
...
textInput(inputId = "exp_features", label = h5("Feature to explain"), value = "")
textInput(inputId = "exp_date", label = h5("Date to explain"), value = "")

actionButton(inputId = "exp_apply_local", label = "Check", icon = icon("play"))
```
...
exp_observation <- eventReactive(
  eventExpr = input$exp_apply_local,
  valueExpr = {
  	req(input$exp_date)
		get_observation(
			data = data_cleaned(), date = lubridate::ymd(input$exp_date), 
			n_assess = input$exp_n_assess, assess_type = input$exp_assess_type
		)
  }
)

exp_features <- eventReactive(
  eventExpr = input$exp_apply_local,
  valueExpr = { 
...
output$exp_features_table <- renderDT({
   tibble::tibble(
   	"Features" = get_features(exp_forecast_results()$fit[[1]], names_only = TRUE)
   ) |> 
		datatable(
      options = list(
...
output$plot_exp_featimp <- renderPlot({
	explain_model(explainer_results(), type = "feature_importance") |> 
		plot()
})

output$plot_exp_varresp <- renderPlot({
	explain_model(
		explainer_results(), type = "variable_response", 
...
output$plot_exp_bd <- renderPlot({
	explain_model(
		explainer_results(), type = "break_down", 
		observation = exp_observation()
	) |> 
		plot()
})
...
output$plot_exp_locstab <- renderPlot({
	explain_model(
		explainer_results(), type = "local_stability", 
		observation = exp_observation(), features = exp_features()
	) |> 
		plot()
})
```
