  ...
  data_selected() |>
      datatable(
        options = list(
          ...
          info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 575 # 250
        )
      )
})
...
data_features <- reactive({
	...
	data_transformed() |> generate_features(params = input, n_future = input$feat_n_future)
})
...
output$feat_str <- renderPrint({ glimpse(data_features()) })
output$feat_summ <- renderPrint({ skim(data_features()) })

output$feat_ts_plot <- renderPlotly({
  req(data_features())
	if (ncol(data_features()) <= 4) {
		data_features() |> 
			timetk::plot_time_series(
				.date_var = date, .value = value,
		...
		data_features() |> 
			dplyr::mutate(dplyr::across(5:ncol(data_features()), as.numeric)) |>
...
br()
h6(
	"Feature selection removes the features that are least significant based on:"
)
prettyRadioButtons(
  inputId = "featsel_cormethod", label = "Correlation Type", 
...
prettyRadioButtons(
  inputId = "featsel_relimp", label = "Importance Type", 
  choices = c("absolute", "relative"), selected = "relative", inline = TRUE
)
sliderInput(
	inputId = "featsel_imp_thresh", label = "Relative Importance Threshold",
...
data_importance <- reactive({
	...
	if (ncol(data_features_full()) > 4) {
		corr_values <- data_features_full() |> 
			generate_correlations(cor_method = input$featsel_cormethod, full_matrix = FALSE)
		pps_values <- data_features_full() |> generate_pps()
		imp_values <- data_features_full() |> 
			generate_model_importance(
				method = c("LASSO", "Random Forest"), importance_type = input$featsel_relimp
			)
...
data_feat_selected <- reactive({
	req(data_importance())
	data_importance() |> select_features(params = input, data_features = data_features_full())
})
...
dropdownButton(
  ...
  awesomeCheckbox(inputId = "use_feat_set", label = "Use selected fetures?", value = TRUE),
  icon = icon("gear"), width = "200px", size = "sm", circle = TRUE,
  tooltip = tooltipOptions(title = "Click to see inputs!")
)
