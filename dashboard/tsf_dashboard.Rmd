...
prettyRadioButtons(
  ...
  choices = c("pearson", "spearman"), selected = "spearman"
)
prettyRadioButtons(
  inputId = "featsel_relimp", label = "Importance Type", 
  choices = c("absolute", "relative"), selected = "relative"
)

br()
h4("Feature Selection")
h6(
	"Activating feature selection removes the features that are least significant based on:"
)
sliderInput(
...
data_importance <- reactive({
	req(data_features()) 
	if (ncol(data_features()) > 4) {
		corr_values <- data_features() |> 
			generate_correlations(cor_method = input$featsel_cormethod, full_matrix = FALSE)
		pps_values <- data_features() |> generate_pps()
		imp_values <- data_features() |> 
...
data_feat_selected <- reactive({
	req(data_importance())
	data_importance() |> select_features(params = input, data_features = data_features())
})
