...
output$feat_table <- renderPrint({ glimpse(features()) })
```
...
```{r}
verbatimTextOutput(outputId = "feat_table")
```
...
output$xfeat_str <- renderPrint({ glimpse(xfeatures()) })
output$xfeat_summ <- renderPrint({ skim(xfeatures()) })
...
output$xfeat_crosscorr_plot <- renderPlotly({
  ...
  data_transformed() |>
    add_future_frame(input$xfeat_n_future) |> 
  	...
  	tidyr::drop_na() |> 
    timetk::plot_acf_diagnostics(
    	.date_var = date, .value = value, .ccf_vars = get_features(xfeatures(), names_only = TRUE)[-1],
    	.lags = ifelse(ts_freq() < 60, ts_freq() * 3, 150),
    	.show_white_noise_bars = TRUE, .show_ccf_vars_only = TRUE,
    	.interactive = TRUE, .title = NULL, .y_lab = NULL,
    	.facet_ncol = ifelse(length(get_features(xfeatures(), names_only = TRUE)[-1]) > 3, 2, 1)
    )
})
```

Row 1 {data-height=100 .tabset}
---------------------------------------------------------------------------
...
dropdownButton(
  numericInput(
    inputId = "selfeat_n_future", label = "Forecast Horizon", 
    value = 12, min = 1, max = Inf, step = 1, width = "100%"
  ),
  icon = icon("gear"), width = "200px", size = "sm", circle = TRUE,
  tooltip = tooltipOptions(title = "Click to see inputs!")
