...
switchInput(inputId = "featsel_filter", value = FALSE, size = "mini")
```

```{r}
corr_values <- reactive({
	req(data_features()) 
	if (ncol(data_features()) > 4) {
		data_features() |> generate_correlations(cor_method = input$featsel_cormethod)
	}
})

ppsr_values <- reactive({
	req(data_features()) 
	if (ncol(data_features()) > 4) {
		data_features() |> 
...
imp_values <- reactive({
	...
	if (ncol(data_features()) > 4) {
		purrr::map(
			...
			~ fit_feature_model(data = data_features(), method = .x)
		) |>  
		purrr::set_names(c("LASSO", "Random Forest")) 
	}
})

data_features_final <- eventReactive(
	eventExpr = input$featsel_filter, 
	valueExpr = {
		req(ppsr_values())
		req(imp_values())
		# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
output$featsel_lasso_plot <- renderPlotly({
	req(imp_values())
	imp_values()$LASSO |> 
		extract_feature_importance(method = "LASSO", relative = input$featsel_relimp) |> 
...
output$featsel_rf_plot <- renderPlotly({
	req(imp_values())
	imp_values()$`Random Forest` |> 
		extract_feature_importance(method = "Random Forest", relative = input$featsel_relimp) |> 
		plot_feature_importance() |> 
		plotly::ggplotly()
})
```
...
```{r}
# mettere il glimpse del feature set filtrato
```
