...
exp_forecast_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$exp_method, n_future = input$exp_n_future,
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateDateInput(
      ...
      value = dplyr::slice_tail(data_cleaned(), n = input$exp_n_assess)$date[1], 
      min = dplyr::slice_tail(data_cleaned(), n = input$exp_n_assess)$date[1], 
      max = dplyr::slice_tail(data_cleaned(), n = input$exp_n_assess)$date[input$exp_n_assess]
    )
  }
)
...
output$plot_exp_bd <- renderPlot({
	...
	exp_obs <- get_observation(
		data = data_cleaned(), date = input$exp_date, method = input$exp_method,
		n_assess = input$exp_n_assess, assess_type = input$exp_assess_type
	)
	explainer_results() |> 
