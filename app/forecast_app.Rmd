...
br()
selectInput(
  inputId = "frequency", label = h5("Dataset frequency"), 
...
br()
shinyWidgets::awesomeCheckbox(inputId = "featsel_save", label = "Use selected feature set?", value = FALSE)
...
data_features_full <- reactive({
	if (!is.null(react_val$uploaded_xfeatures)) {
		data_features() |> dplyr::left_join(data_xfeatures(), by = c("date", "id", "frequency"))
	} else {
		data_features()
	}
})
...
data_feat_selected <- reactive({
	...
	n_feat <- get_features(data_features_full(), remove_date = TRUE, number_only = TRUE)
	if (n_feat > 0) {
		data_features_full() |> 
