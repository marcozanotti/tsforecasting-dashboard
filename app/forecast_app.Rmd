...
data_selected <- reactive({
	req(data_loaded())
	if (input$impute) {
		data_loaded() |> dplyr::arrange(date) |> impute_data(impute = input$impute)
	} else {
  	data_loaded() |> dplyr::arrange(date)
  }
})
...
data_params <- reactive({ 
	req(data_loaded()) 
	n <- nrow(data_loaded())
	freq <- ifelse(input$frequency == "Auto", timetk::tk_get_frequency(data_loaded()[["date"]]), parse_frequency(input$frequency)) 
	date_min <- min(data_loaded()[["date"]])
	date_max <- max(data_loaded()[["date"]])
	acf_n_lags <- parse_acf_lags(freq)
	horizon <- set_horizon(freq)
	n_assess <- round(n * 0.2) # 20% of data
	date_assess_min <- tail(data_loaded()[["date"]], n = horizon + n_assess)[1]
	date_assess_max <- tail(data_loaded()[["date"]], n = horizon + n_assess)[n_assess]
	list(
		"n" = n, "freq" = freq, "date_min" = date_min, "date_max" = date_max, 
		"acf_n_lags" = acf_n_lags, "horizon" = horizon, "n_assess" = n_assess,
		"date_assess_min" = date_assess_min, "date_assess_max" = date_assess_max
	)
})
...
data_test <- reactive({
  data_selected() |> 
    transform_data(transformations = input$test_transf,	frequency = data_params()$freq) |> 
...
data_anomaly <- reactive({ detect_anomalies(data_selected(), params = input) })
data_cleaned <- reactive({ clean_anomalies(data_selected(), data_anomaly()) })
...
transform_params <- reactive({
	data_cleaned() |> 
    transform_data(transformations = input$transf,	frequency = data_params()$freq) |> 
...
# update date range when dataset or data_import
observeEvent(
	eventExpr = input$dataset, 
	handlerExpr = {
		shinyjs::reset("frequency")
		updateNumericInput(session = session, inputId = "horizon", value = data_params()$horizon)
		updateNumericInput(session = session, inputId = "n_assess", value = data_params()$n_assess)
		updateDateRangeInput(
			session = session, inputId = "transf_date_range",
			start = data_params()$date_min, end = data_params()$date_max,
			min = data_params()$date_min, max = data_params()$date_max
		)
		updateDateInput(
			session = session, inputId = "exp_date",
			value = data_params()$date_assess_min,
			min = data_params()$date_assess_min, max = data_params()$date_assess_max
		)
})
observeEvent(
	eventExpr = input$data_import, 
...
observeEvent(
	...
	handlerExpr = {
		updateNumericInput(session = session, inputId = "horizon", value = set_horizon(parse_frequency(input$frequency)))
})
```
