...
pkgs <- c(
	"modeltime", "modeltime.h2o", "rules", "yardstick", "timetk"
)
...
<img src="tsf_logo.png" width="200" alt="Forecast App Logo"/>

<p style="font-size:18px; max-width:600px; margin:auto; margin-top:20px;">
<b>Welcome to the Forecast App!</b>
<br>
This app provides a complete framework for analyzing, modeling, and
forecasting time series data.
It guides the user through data preparation, exploratory analysis,
feature engineering, model training, evaluation, and scenario building.
</p>

<p style="font-size:16px; margin-top:20px;">
<b>Author:</b> <a href="https://marcozanotti.netlify.app/" target="_blank">Marco Zanotti</a><br>
<b>Licensed under:</b> <a href="https://opensource.org/licenses/MIT" target="_blank">MIT License</a>
</p>
...
# 	updateTabsetPanel(session, inputId = "main_tabs", selected = "data_section")
# })
```
</div>
...
h6("The dataset must be a .csv file (sep = ',', dec = '.') containing columns 'date', 'id', and 'value'.")
fileInput(
...
selectInput(
  inputId = "frequency", label = h5("Select dataset frequency"), 
  ...
  data_selected() |>
      DT::datatable(
        options = list(
          ...
          info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 575, scrollX = TRUE
        )
      )
})
...
shinyWidgets::switchInput(inputId = "impute", value = FALSE, size = "mini")
...
hr()
h4("Autocorrelation Tests")
h5("Ljung-Box and Box-Pierce")
br()

hr()
h4("Stationarity Tests")
h5("ADF, PP and KPSS")
...
data_anomaly <- reactive({
  data_imputed() |> 
		anomaly_detection_and_cleaning(params = input)
})
...
output$anomaly_table <- DT::renderDT({
  data_anomaly() |> 
    ...
    DT::datatable(
      options = list(
        ordering = TRUE, pageLength = 10, lengthChange = TRUE, searching = TRUE, 
        info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 200, scrollX = TRUE
      ), 
      rownames = FALSE
    )
})
...
shinyWidgets::dropdownButton(
  numericInput(
    inputId = "feat_n_future", label = "Forecast Horizon", 
...
textInput(
	inputId = "feat_fourier_p", label = "Fourier Periods", value = "", placeholder = "Values separated by a comma..."
)
sliderInput(
	inputId = "feat_fourier_k", label = "Fourier Orders",
	min = 1, max = 10, value = 1, step = 1
)
textInput(
	inputId = "feat_spline_deg", label = "Spline Degrees", value = "", placeholder = "Values separated by a comma..."
)
textInput(
	inputId = "feat_lag", label = "Lag Orders", value = "", placeholder = "Values separated by a comma..."
)
textInput(
	inputId = "feat_roll", label = "Lag Rolling Periods", value = "", placeholder = "Values separated by a comma..."
)
textInput(
	inputId = "feat_inter", label = "Interactions", value = "", placeholder = "Expressions separated by a comma..."
)
shinyWidgets::prettyRadioButtons(
  inputId = "feat_table_type", label = h5("Show:"), 
  choices = c("Preview", "Structure", "Summary"), selected = "Structure"
)
...
data_features <- reactive({
	...
	data_transformed() |> 
		generate_features(params = input, n_future = input$feat_n_future, verbose = 0)
})
...
output$feat_table <- DT::renderDT({
  if (get_features(data_features(), number_only = TRUE) > 1) {
      ...
      DT::datatable(
        options = list(
          ordering = TRUE, pageLength = 20, lengthChange = TRUE, searching = TRUE, 
          info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 550, scrollX = TRUE
        )
      )
  }
})

output$feat_str <- renderPrint({ 
	if (get_features(data_features(), number_only = TRUE) > 1) {
  	dplyr::glimpse(data_features()) 
  }
})

output$feat_summ <- renderPrint({ 
	if (get_features(data_features(), number_only = TRUE) > 1) {
  	skimr::skim(data_features()) 
  }
})
...
```{r}
conditionalPanel(
  condition = "input.feat_table_type == 'Preview'",
  DT::DTOutput(outputId = "feat_table")
)
conditionalPanel(
  condition = "input.feat_table_type == 'Structure'",
  verbatimTextOutput(outputId = "feat_str")
)
conditionalPanel(
  condition = "input.feat_table_type == 'Summary'",
...
shinyWidgets::dropdownButton(
  numericInput(
    inputId = "xfeat_n_future", label = "Forecast Horizon", 
    value = 12, min = 1, max = Inf, step = 1, width = "100%"
  ),
  icon = icon("gear"), width = "225px", size = "sm", circle = TRUE,
  tooltip = shinyWidgets::tooltipOptions(title = "Click to see inputs!")
...
shinyWidgets::prettyRadioButtons(
  inputId = "xfeat_table_type", label = h5("Show:"), 
  choices = c("Preview", "Structure", "Summary")
)
...
output$xfeat_table <- DT::renderDT({
  data_xfeatures() |>
      DT::datatable(
        options = list(
          ordering = TRUE, pageLength = 20, lengthChange = TRUE, searching = TRUE, 
          info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 550 # 250
        )
      )
})
...
output$xfeat_ts_plot <- plotly::renderPlotly({
	req(data_xfeatures())
  data_transformed() |>
    add_future_frame(input$xfeat_n_future) |> 
  	dplyr::left_join(data_xfeatures(), by = c("date", "id", "frequency")) |> 
...
```{r}
conditionalPanel(
  condition = "input.xfeat_table_type == 'Preview'",
  DT::DTOutput(outputId = "xfeat_table")
)
conditionalPanel(
  condition = "input.xfeat_table_type == 'Structure'",
  verbatimTextOutput(outputId = "xfeat_str")
)
conditionalPanel(
  condition = "input.xfeat_table_type == 'Summary'",
...
shinyWidgets::dropdownButton(
  numericInput(
    inputId = "featsel_n_future", label = "Forecast Horizon", 
    value = 12, min = 1, max = Inf, step = 1, width = "100%"
  ),
  icon = icon("gear"), width = "225px", size = "sm", circle = TRUE,
  tooltip = shinyWidgets::tooltipOptions(title = "Click to see inputs!")
...
br()
shinyWidgets::awesomeCheckbox(inputId = "featsel_save", label = "Save feature set?", value = FALSE)
br()
shinyWidgets::awesomeCheckbox(
	inputId = "featsel_norm", label = "Plot normalized values?", value = FALSE
)
br()
sliderInput(
	inputId = "featsel_cor_thresh", label = "Correlation Threshold",
	min = 0, max = 1, value = 0, step = 0.01
)
sliderInput(
	inputId = "featsel_pps_thresh", label = "PPS Threshold",
	min = 0, max = 1, value = 0, step = 0.01
)
sliderInput(
	inputId = "featsel_lasso_thresh", label = "LASSO Threshold",
	min = 0, max = 1, value = 0, step = 0.01
)
sliderInput(
	inputId = "featsel_rf_thresh", label = "Random Forest Threshold",
	min = 0, max = 1, value = 0, step = 0.01
)
br()
textInput(
	inputId = "featsel_reg_formula", label = "Regression Formula", 
...
data_features_full <- reactive({
	...
	if (!is.null(react_val$uploaded_xfeatures)) {
		req(data_xfeatures())
		data_features() |> dplyr::left_join(data_xfeatures(), by = c("date", "id", "frequency")) # c("date", "id", "frequency")
	} else {
		data_features()
	}
})

data_importance <- reactive({
	req(data_features_full()) 
	n_feat <- length(get_features(data_features_full(), names_only = TRUE))
	if (n_feat > 1) {
		corr_values <- data_features_full() |> 
			generate_correlations(cor_method = "spearman", full_matrix = FALSE)
		pps_values <- data_features_full() |> generate_pps()
		imp_values <- data_features_full() |> 
			generate_model_importance(method = c("LASSO", "Random Forest"))
		c(list("Correlation" = corr_values, "PPS" = pps_values), imp_values) |> 
			dplyr::bind_rows()
	}
})
...
data_feat_selected <- reactive({
	...
	data_importance() |> 
		select_features(
			...
			n_future = input$featsel_n_future
		)
})

n_features <- reactive({
	req(data_feat_selected())
	get_features(data_feat_selected(), remove_date = TRUE, number_only = TRUE)
})

output$featsel_corr_plot <- plotly::renderPlotly({
	req(data_importance())
	if (input$featsel_norm) {
		g <- data_importance() |> 
			dplyr::filter(type == "Correlation") |> 
			plot_feature_importance(normalized = TRUE) + ggplot2::expand_limits(x = c(0, 1))
	} else {
		g <- data_importance() |> 
			dplyr::filter(type == "Correlation") |> 
			plot_feature_importance(normalized = FALSE) + ggplot2::expand_limits(x = c(-1, 1))
	}
	plotly::ggplotly(g)
})
...
# 	req(data_importance())
# 	g <- data_features() |> 
#			generate_correlations(cor_method = input$featsel_cormethod, full_matrix = TRUE) |> 
...
output$featsel_ppsr_plot <- plotly::renderPlotly({
	...
	g <- data_importance() |> 
		...
		plot_feature_importance(normalized = input$featsel_norm) + ggplot2::expand_limits(x = c(0, 1))
	plotly::ggplotly(g)
})
...
shinyWidgets::awesomeCheckbox(inputId = "use_feat_set", label = "Use selected fetures?", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    shinyWidgets::updateNumericInput(session = session, inputId = "n_future", value = 12)
    shinyWidgets::updateNumericInput(session = session, inputId = "n_assess", value = 24)
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "assess_type", selected = "Rolling")
    shinyWidgets::updateAwesomeCheckbox(session = session, inputId = "use_feat_set", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "tune_assess_type", selected = "Rolling")
...
shinyWidgets::awesomeCheckbox(inputId = "comp_tune", label = "Use optimized algorithms?", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "comp_assess_type", selected = "Rolling")
    shinyWidgets::updatePickerInput(session = session, inputId = "comp_method", selected = c("ETS", "SARIMA"))
    shinyWidgets::updateAwesomeCheckbox(session = session, inputId = "comp_tune", value = FALSE)
...
compare_results <- eventReactive(
  ...
  valueExpr = {
  	...
  	if (input$comp_tune) {
  		...
  		if (length(comp_no_tune_mtd) > 0) {
  			for (mtd in comp_no_tune_mtd) {
  				react_val$comp_fit[[mtd]] <- fit_model(
  					data = data_transformed(), method = mtd, params = input,
  		...
  		react_val$comp_fit |> 
  			generate_forecast(
  				data = data_transformed(), method = input$comp_method,
  				n_future = input$comp_n_future,	n_assess = input$comp_n_assess,
  				assess_type = input$comp_assess_type
  			)
  	} else {
  		purrr::map(
  			input$comp_method,
...
shinyWidgets::awesomeCheckbox(inputId = "exp_tune", label = "Use optimized algorithm?", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "exp_assess_type", selected = "Rolling")
  ...
  valueExpr = {
  	...
  	if (input$exp_tune) {
  		generate_model_explainer(
  			fitted_model = react_val$tune_fit[[input$exp_method]],
  			data = data_transformed(), method = input$exp_method,	params = input,
...
shinyWidgets::awesomeCheckbox(inputId = "ens_tune", label = "Use optimized algorithms?", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "ens_assess_type", selected = "Rolling")
    shinyWidgets::updatePickerInput(session = session, inputId = "ens_method", selected = c("ETS", "SARIMA"))
    shinyWidgets::updateAwesomeCheckbox(session = session, inputId = "ens_tune", value = FALSE)
...
shinyWidgets::awesomeCheckbox(inputId = "scn_tune", label = "Use optimized algorithm?", value = FALSE)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    shinyWidgets::updatePrettyRadioButtons(session = session, inputId = "scn_assess_type", selected = "Rolling")
    updateSelectInput(session = session, inputId = "scn_method", selected = "Ensemble")
    shinyWidgets::updateAwesomeCheckbox(session = session, inputId = "scn_tune", value = FALSE)
  	...
  	if (input$scn_method == "Ensemble") {
  		...
  		if (input$scn_tune) {
  			...
  			react_val$ens_fit |>
  				generate_forecast(
  					data = data_transformed(), method = input$ens_method,
  					n_future = input$scn_n_future, n_assess = input$scn_n_assess,
  					assess_type = input$scn_assess_type, ensemble_methods = input$ens_type,
  		...
  		if (input$scn_tune) {
  			...
  			react_val$ens_fit |>
  				generate_forecast(
  					data = data_transformed(), method = input$ens_method,
  					n_future = input$scn_n_future, n_assess = input$scn_n_assess,
  					assess_type = input$scn_assess_type, ensemble_methods = NULL,
  		...
  		if (input$scn_tune) {
  			list(react_val$tune_fit[[input$scn_method]]) |>
  				generate_forecast(
  					data = data_transformed(), method = input$scn_method,
  					n_future = input$scn_n_future, n_assess = input$scn_n_assess,
  					assess_type = input$scn_assess_type, confidence_level = c(0.05, 0.99)
  				)
  		} else {
  			purrr::map(
  				input$scn_method,
...
```{r}
observeEvent(
  eventExpr = input$featsel_save,
  handlerExpr = {
    req(data_feat_selected())
  	if (get_features(data_feat_selected()$data, remove_date = TRUE, number_only = TRUE) > 0) {
  		shinyWidgets::updateAwesomeCheckbox(inputId = "use_feat_set", session = session, value = TRUE)
  		updateNumericInput(inputId = "n_future", session = session, value = input$featsel_n_future)
  	}
  }
)
```
