---
title: "Forecast Dashboard - Manual"
...
The **Forecast Dashboard** is a comprehensive tool designed to guide users through the entire forecasting workflow, from raw data to scenario analysis. It is tailored for both beginners and experienced practitioners, allowing the exploration, modeling, and explanation of time series data without requiring advanced knowledge of forecasting techniques.  

The dashboard is organized into four main sections —**Data**, **Analyze**, **Features**, and **Forecast**— which collectively enable users to:  
...
All actions performed in the dashboard are **automatically saved**, meaning that any transformation, cleaning, or feature engineering applied to the series is carried forward to the subsequent steps. This ensures a consistent and reproducible workflow from raw data to final forecasts.  

The dashboard also provides great flexibility in analysis: interactive visualizations, configurable model parameters, and multiple options for evaluation and explanation allow users to tailor the process to their specific needs. Common controls, such as the **gear button** ⚙️, enable the selection of forecast horizon and assessment periods, ensuring that every analysis can be customized for optimal insights. The **play button** ▶️ is used to execute actions like generating forecasts or running tests, while a **reset button** 🔄 is often available to clear all settings and start fresh.

Follow this user guide to leverage the full potential of the Forecast Dashboard to obtain accurate forecasts, understand model behavior, and explore different scenarios for informed decision-making.  

If you are interested in who I am and what I do, visit my [website](https://marcozanotti.netlify.app/).   

The [source code](https://github.com/marcozanotti/tsforecasting-dashboard) of the Forecast Dashboard is available on GitHub under the MIT license.  
...
Feel free to share the Forecast Dashboard on LinkedIn if you find it useful and remember to tag [me](https://www.linkedin.com/in/marco-zanotti-a6a978124/)!  



## Section 1: Data

The **Data** section is the starting point of the Forecasting Dashboard. Here you can choose which time series to analyze and prepare for forecasting.  

#### Predefined datasets
The dashboard includes a few **predefined univariate time series datasets** that you can load instantly. These datasets are useful for exploring the dashboard’s functionalities and testing the workflow without needing to upload your own data.  

#### Import your own data
You can also import your own **univariate time series dataset** in CSV format.  
...
- **`frequency`**: the frequency of the series (must be one of the allowed values listed below)  
...
- `bus-hour` (252 × 24 observations per year, business hours)  
- `hour` (365 × 24 observations per year, calendar hours)  
- `bus-half-hour` (252 × 48 observations per year, business half-hours)  
- `half-hour` (365 × 48 observations per year, calendar half-hours)  

> ⚠️ **Note**: Make sure your dataset is properly formatted, as incorrect column names or unsupported frequency values will prevent the dashboard from processing the data.  

#### Explore the data
Once a dataset is loaded, you can briefly explore it using three buttons available in this section:  
...
#### Options available
When visualizing the data, you can adjust several parameters:  

- **Select time period**: choose a start and end date to focus on a specific portion of the series.  
- **Add smooth line**: overlay a smoothed curve on the time series plot to highlight the underlying trend.  
- **ACF lags**: specify the number of lags to display in the Autocorrelation Function (ACF) plot.  
- **Impute missing values**: apply automatic interpolation based on the declared frequency to handle missing observations before plotting.  

#### Panels displayed
Four plots are displayed simultaneously in a 2 × 2 layout:  

1. **Time series plot** (top-left): shows the raw time series (with optional smoothing and imputation).  
2. **Autocorrelation plot (ACF)** (top-right): displays correlations between the series and its lagged values.  
3. **Decomposition plot** (bottom-left): breaks down the series into trend, seasonal, and residual components.  
4. **Seasonality plot** (bottom-right): visualizes recurring seasonal patterns within the chosen frequency.  
...
#### Tests performed
The dashboard groups the statistical tests into three categories:  
...
- **Confidence level**: set the statistical threshold for identifying anomalies.  
- **Maximum % of anomalies**: limit the proportion of points flagged as anomalies.  
- **Clean anomalies**: choose whether to automatically remove or adjust the detected anomalies.  
...
2. **Cleaned time series**: displays the series after anomalies have been removed or adjusted (if the cleaning step was triggered).  
...
- **Smooth line**: add a smoothing line to visualize trends.  
- **Number of lags for ACF plot**: adjust the lag parameter for the autocorrelation plot.  
- **Missing value imputation**: trigger automatic interpolation based on the series frequency.  
...
- **Feature visualization**: choose how to explore the generated features: table, structure, or summary statistics.  

#### Panels displayed
1. **Features results**: shows the features in a table, structure, or summary statistics.  
2. **Time series plot with features**: displays the original series together with all added features for visual inspection.  
...
#### Options available
- **Upload external dataset**: the dataset must follow the same structure as in Section 1, with columns `date`, `id`, `frequency`, and one or more feature columns.  
- **Feature visualization**: choose how to explore the uploaded features: table, structure, or summary statistics.  

> ⚠️ **Note:** External features must be available in the future to be used in forecasting.  

#### Panels displayed
1. **Features results**: shows the external features in a table, structure, or summary statistics.  
...
#### Options available
- **Absolute Correlation Threshold**: set the minimum absolute correlation value for a feature to be retained.  
...
5. **Selected feature sets**: displays features retained by each selection method.  
...
> ⚙️ **General Options:*** In all subsections of the Forecast section, a gear button is available that allows you to specify the **forecast horizon**, the **assessment period**, and the **assessment type** (expanding or rolling window).  
...
> ⚙️ **General Options:** In the gear button you can specify whether to use the selected features or not.
...
- **Step 1: Select Algorithm**
  ...
  - **Use optimized algorithm?**: toggle to use the hyperparameter-optimized version if available.  
  - **Back-transform**: choose whether to back-transform forecasts if a transformation was applied to the time series.  
...
- **Optimize stack?**: toggle to perform hyperparameter tuning for the stacking model.  
...
- **Table of aggregate results**: aggregated forecast values over the horizon, with comparisons to the previous period and previous seasonal period.  
...
> ⚠️ **Note:** The worst and best case scenarios are not affected by the manual adjustment.
